...
<div class="row clearfix aboutappcontainer">
  ...
  <div class="card-body">
    @if(!is_null($about_app))
    <form method ="POST" action="{{route('app.aboutapp.update',$about_app->id)}}" enctype="multipart/form-data" name="registration">
     @csrf
     {{ method_field('PUT') }}
    @else
    <form method ="POST" action="{{route('app.aboutapp.store')}}" enctype="multipart/form-data" name="registration">
      @csrf
    @endif
