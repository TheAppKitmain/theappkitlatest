            ...
            <div class="payment-wrapper-inner">
               <div class="container-fluid">
                ...
                <h2 class="billing-title-top">Billing</h2>
                  ...
                  <div class="pricing-box-myplan">
                     <h4>Confirm your new plan</h4>
                     <div class="pricing-box-ch">
                     <form method="post" action="{{route('theme.update_subscription')}}">
                     @csrf
                     <input type="hidden" name="template_id" value="{{$template_id}}">
                     <input type="hidden" name="plan_id" value="{{$plans_retrieve->id}}"> 	
                     <div class="pricing-box-ch-inner">
                     	<span class="changetopan">Changing to</span>
                           <label for="planmonth" class="d-flex radio-planpayemnt pralitive">
                           <!-- <h5 class="plantitleh updatemnthlypl">Monthly Plan</h5> -->
                              ...
                              <div class="bootom-total-price-right right-pric">
                                 @if($usercountry->country == 'United Kingdom')
                                    <!-- <h6 class="h6_dy_currency">{{$plans_retrieve->currency}}</h6> -->
                                    <h6>Every {{$plans_retrieve->interval_count}} {{$plans_retrieve->interval}}</h6>
                                    <h6>@if($plans_retrieve->currency =='gbp') £@endif{{number_format((float)$plans_retrieve->amount/100, 2, '.', '')}}</h6>
                                 @else
                                    <!-- <h6 class="h6_dy_currency">{{$plans_retrieve->currency}}</h6> -->
                                    <h6>Every {{$plans_retrieve->interval_count}} {{$plans_retrieve->interval}}</h6>
                                    <h6>@if($plans_retrieve->currency =='usd') $@endif{{number_format((float)$plans_retrieve->amount/100, 2, '.', '')}}</h6>
                                 @endif
                              </div>
                           </label>
                       <div class="planbuttons">
                      <button type="submit" class="update-plan-btn">Confirm</button> 	
                      <a class="cancel-plan-btn" href="{{route('theme.update_subscription_data')}}">Go back</a>
                     </div>
                     </div>
                 </form>
                        </div>
